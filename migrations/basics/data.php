<?php
/**
 * bbGuild LOTRO plugin - Data seeding migration
 *
...
class data extends \phpbb\db\migration\container_aware_migration
{
	...
	public function effectively_installed()
	{
		...
		$sql = 'SELECT COUNT(*) AS cnt FROM ' . $games_table . " WHERE game_id = 'lotro'";
	...
	{
		$installer = $this->container->get('avathar.bbguild_lotro.installer');
		$installer->install($this->get_table_names(), 'lotro', 'Lord of the Rings Online', '', '', 'us');
	}

	public function remove_game_data()
	{
		$installer = $this->container->get('avathar.bbguild_lotro.installer');
		$installer->uninstall($this->get_table_names(), 'lotro', 'Lord of the Rings Online');
	}
